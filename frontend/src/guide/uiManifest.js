...
export function guideReply(question, context = {}) {
  ...
  if (/(graph|map|connect|relationship|contradict)/.test(text)) return { text: context.documents > 0 ? 'Open Knowledge map to inspect evidence-backed relationships. Select a claim or edge to see its provenance; correlation edges do not imply causation.' : 'The map becomes useful after documents provide claims, entities, events, or structured variables.', target: 'knowledge_map' };
